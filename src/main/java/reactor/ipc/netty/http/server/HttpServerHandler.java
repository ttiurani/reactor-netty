...
final class HttpServerHandler extends ChannelDuplexHandler
	...
	final ConnectionEvents parentListener;
	...
	HttpServerHandler(ConnectionEvents parentListener) {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		if (msg instanceof HttpRequest) {
			...
			if (overflow || pendingResponses > 1) {
				...
				overflow = false;
				parentListener.onSetup(ctx.channel(), msg);
	...
	public void run() {
		...
		while ((next = pipelined.peek()) != null) {
			if (next instanceof HttpRequest) {
				...
				nextRequest = true;
				parentListener.onSetup(ctx.channel(), next);
